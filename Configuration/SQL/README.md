...
| 16 | `016_cfg_choice_fields_align.sql` | Aligns the registry/map to the authoritative TSS Choice Fields reference (35 fields): removes the invalid `transport_charges`/`controlled_goods_type`/`package_type`, fixes header `movement_type` → CV `movement_type` (not `mode_of_transport`), and adds `UsedBy` + descriptions per the reference. |
