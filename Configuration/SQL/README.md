...
| 17 | `017_cfg_choice_value_widen.sql` | Widens `CFG.Choice_Value_Cache` for large reference sets: `ChoiceName` → `nvarchar(max)`, `ChoiceValue` → `nvarchar(255)` (UQ rebuilt). Prevents "string would be truncated" on long codes/descriptions (e.g. `additional_procedure_code`, `commodity_code`). |
