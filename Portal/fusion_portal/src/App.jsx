...
function normalizeConsignment(row) {
  ...
  const consignmentNumber = row.ConsignmentNumber ?? row.consignmentNumber ?? '';
  const fallbackNumber = consignmentRowId ? `PRS-${consignmentRowId}` : 'PRS-DRAFT';
  // CWF keeps no numeric consignment id, so keying rows on it would collapse
  // every row onto the same id. The DEC reference is unique in every tenant.
  const rowKey = consignmentNumber || (consignmentRowId ? `PRS-C${String(consignmentRowId).padStart(6, '0')}` : 'PRS-C-DRAFT');
  return {
    id: rowKey,
...
function ConsignmentDetailModal({ row, onClose, onSave, onQueueForTss }) {
  ...
  useEffect(() => {
    ...
    // The DEC reference is the key the mirror shares across every tenant; the
    // numeric row id only exists for some of them.
    const detailReference = row?.consignmentNumber || row?.consignmentRowId;
    if (!detailReference) {
    ...
    getConsignmentDetail(detailReference, row?.clientCode)
      .then((payload) => {
        if (cancelled) return;
