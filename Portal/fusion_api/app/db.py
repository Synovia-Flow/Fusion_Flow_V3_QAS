...
from .config import ConfigError, connection_string, db_config, mirror_connection_string, raw_connection_string
...
def _connection_options(raw: str = "") -> dict[str, str]:
    raw = raw or raw_connection_string()
    if raw:
...
def _connect_pytds(raw: str = "") -> _PytdsConnection:
    try:
    ...
    options = _connection_options(raw)
...
@contextmanager
def connect(raw: str = ""):
    """Open a connection. `raw` targets a database other than the primary one."""
    try:
        ...
        try:
            conn = pyodbc.connect(raw or raw_connection_string() or connection_string(db_config()), autocommit=True, timeout=10)
...
def _mirror_target() -> str:
    target = mirror_connection_string()
    if not target:
        raise DbUnavailable(
            "The tenant mirror database is not configured. Set DB_CONN_STR_MIRROR to the Fusion_Flow_V3 connection."
        )
    return target


def mirror_query_all(sql: str, params: Iterable[Any] = ()) -> list[dict[str, Any]]:
    with connect(_mirror_target()) as conn:
        cursor = conn.cursor()
        cursor.execute(sql, tuple(params))
        return rows_to_dicts(cursor)


def mirror_query_one(sql: str, params: Iterable[Any] = ()) -> dict[str, Any] | None:
    rows = mirror_query_all(sql, params)
    return rows[0] if rows else None


def mirror_execute_scalar(sql: str, params: Iterable[Any] = ()) -> Any:
    with connect(_mirror_target()) as conn:
        cursor = conn.cursor()
        cursor.execute(sql, tuple(params))
        row = cursor.fetchone()
