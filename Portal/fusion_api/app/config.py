...
@lru_cache(maxsize=1)
def mirror_connection_string() -> str:
    """Connection to the per-tenant TSS mirror database (Fusion_Flow_V3).

    The portal reads two databases. Configuration and authentication live in the
    Release 1 model (CFG/PRS/STG), which the primary connection serves. The
    operational TSS mirror is a different database whose tenant IS the schema:
    BKD/PLE/CWF/CRS each own Consignments, ENS_Headers, GoodsItems and the SFD/SDI
    tables. Neither database contains the other's objects, so one connection
    string cannot serve both.

    Empty when unconfigured; callers surface that as a 503 rather than guessing.
    """
    env_value = os.environ.get("DB_CONN_STR_MIRROR")
    if env_value:
        return _clean_env_value(env_value)
    return _dotenv_values(DEFAULT_DOTENV).get("DB_CONN_STR_MIRROR", "")
