...
IF OBJECT_ID('CFG.Job', 'U') IS NOT NULL
    ...
    USING (VALUES
        ...
        ('SUB_MIRROR_BKD_ENS', 'Mirror - Birkdale ENS Header from TSS', 'SUBMISSION', 'BKD', NULL, 'TASK', NULL, NULL,
         'Every 30 minutes, for SUBMITTED STG rows with a Declaration_Number, GET the header back from TSS and upsert the authoritative live record into TSS.BKD_ENS_Header (the live mirror). Marks the STG row RECONCILED (complete). Logs to API.Call + EXC RECONCILING -> RECONCILED.',
         'mirror_ens:main', 'STG.BKD_ENS_Header (SUBMITTED) + TSS API', 'TSS.BKD_ENS_Header + STG', 'Every 30 minutes', 1,
         'Module 3 step 3. Periodic TSS status sync every 30 minutes; live mirror the update/cancel jobs link to.'),
